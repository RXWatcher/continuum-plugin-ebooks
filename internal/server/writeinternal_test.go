...
func TestWriteInternal_Opaque(t *testing.T) {
	leak := errors.New(`pq: relation "kosync_user" does not exist; host=db.internal user=silo`)
	...
	if b := w.Body.String(); strings.Contains(b, "kosync_user") || strings.Contains(b, "db.internal") || strings.Contains(b, "silo") {
