...
export default function CustomMetadataProvidersTab() {
  ...
  const providers = useQuery({
    ...
    queryFn: listCustomMetadataProviders,
  });
  ...
  const remove = useMutation({
    mutationFn: (id: string) => deleteCustomMetadataProvider(id),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["admin-custom-metadata-providers"] }),
  ...
  const toggle = useMutation({
    mutationFn: (p: CustomMetadataProvider) =>
      updateCustomMetadataProvider(p.id, { enabled: !p.enabled }),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["admin-custom-metadata-providers"] }),
  ...
  return (
    <Card className="bg-surface p-4">
      ...
      <p className="text-muted-foreground mb-4 text-xs">
        External HTTP search endpoints conforming to the
...
function ProviderEditor({
  ...
  const save = useMutation({
    mutationFn: () =>
      updateCustomMetadataProvider(provider.id, {
        name: name.trim(),
        url: url.trim(),
        auth_header: authHeader.trim(),
        enabled,
