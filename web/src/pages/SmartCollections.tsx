...
function CollectionCard({
  ...
  const ruleCount =
    collection.queryDef?.groups?.reduce((n, g) => n + g.rules.length, 0) ?? 0;
  return (
    <Card className="bg-surface hover:bg-surface-hover group relative p-4 transition-colors">
      <Link
  ...
  return (
    <Card className="bg-surface flex flex-col items-center gap-3 p-10 text-center">
      ...
      <p className="text-muted-foreground max-w-sm text-sm">
        ...
        books, you define filters (genre, author, rating, year, format,
        reading status) and the shelf populates itself.
      </p>
