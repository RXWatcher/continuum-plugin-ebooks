...
import { browseSeries, type FacetItem, type PageEnvelope } from "@/lib/api";
...
export default function Series() {
  ...
  const q = useQuery({
    queryKey: ["browse", "series", cursor, libraryID],
    queryFn: async () => {
      const page = await browseSeries(cursor, 50, libraryID);
    ...
    <div className="space-y-4">
      ...
      {q.isLoading && pages.length === 0 ? (
        ...
        <p className="text-sm text-muted-foreground">
          This backend doesn&rsquo;t support browsing by series &mdash; try
          search instead.
        </p>
      ) : (
        <>
