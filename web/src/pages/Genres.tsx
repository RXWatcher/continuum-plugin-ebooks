...
import { browseGenres, type FacetItem, type PageEnvelope } from "@/lib/api";
...
export default function Genres() {
  ...
  const q = useQuery({
    queryKey: ["browse", "genres", cursor, libraryID],
    queryFn: async () => {
      const page = await browseGenres(cursor, 50, libraryID);
    ...
    <div className="space-y-4">
      ...
      {q.isLoading && pages.length === 0 ? (
        ...
        <p className="text-sm text-muted-foreground">
          This backend doesn&rsquo;t support browsing by genre &mdash; try
          search instead.
        </p>
      ) : (
        <>
