...
// Field catalog matches internal/smartcoll/query.go on the ebook
// plugin: 17 fields total, with format / tag / isbn surfacing here
// that don't exist on the audiobook side. Personalized fields
// (finished, in_progress, last_read, abandoned, annotation_count)
// marked with * in the picker.
...
const FIELDS: { value: string; label: string; ops: string[]; personalized?: boolean }[] = [
  ...
  { value: "tag", label: "Tag", ops: ["is", "is_not", "contains"] },
  { value: "format", label: "Format", ops: ["is", "is_not", "contains"] },
  ...
  { value: "isbn", label: "ISBN", ops: ["is", "is_not"] },
  ...
  { value: "last_read", label: "Last read", ops: ["gt", "gte", "lt", "lte", "between", "in_last"], personalized: true },
  { value: "abandoned", label: "Abandoned", ops: ["is"], personalized: true },
  { value: "annotation_count", label: "Annotation count", ops: ["gt", "gte", "lt", "lte", "between"], personalized: true },
];
...
export default function SmartCollectionDetail() {
  ...
  const preview = useQuery({
    ...
    queryFn: () => getSmartCollectionItems(id, 0, 24),
  ...
  const addGroup = () => {
    setQuery((q) => ({
      ...
      groups: [
        ...q.groups,
        { match: "all", rules: [{ field: "title", op: "contains", value: "" }] },
      ],
    }));
  };
          ...
          <Card className="bg-surface space-y-3 p-4">
            ...
            {query.groups.length === 0 ? (
              <p className="text-muted-foreground text-xs">
                No rules yet — this collection currently matches every ebook in
                your libraries.
              </p>
            ) : (
          ...
          <Card className="bg-surface p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-muted-foreground text-xs">
                {preview.data?.total ?? 0} match
                {preview.data?.total === 1 ? "" : "es"}
              </span>
            </div>
            {preview.isLoading ? (
              ...
              <div className="space-y-2">
                {preview.data!.items.slice(0, 12).map((book: EbookSummary) => (
                  <div
                    key={book.id}
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Input
        ...
        onChange={(e) => {
          ...
          if (["year", "rating", "annotation_count"].includes(rule.field)) {
