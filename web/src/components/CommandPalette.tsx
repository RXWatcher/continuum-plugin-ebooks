...
function Palette({ close }: { close: () => void }) {
  ...
  const commands = useMemo<Cmd[]>(() => {
    const navCommands: Cmd[] = [
      ...
      { id: "nav-stats", label: "My stats", icon: <Flame className="size-4" />, perform: () => navigate("/me/stats") },
      { id: "nav-settings", label: "Settings", icon: <Bell className="size-4" />, perform: () => navigate("/me/settings") },
